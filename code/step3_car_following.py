...
Usage:
    # 处理单个时间段
    python code/step3_car_following.py --period 0750am-0805am --data-dir US-101

    # 处理所有时间段
    python code/step3_car_following.py --data-dir US-101
...
def main():
    ...
    if period:
        ...
        INPUT_FILE = os.path.join(args.output_dir, data_dir_name, period, f'trajectories_cleaned_{period}.csv')
    else:
        OUTPUT_DIR = os.path.join(args.output_dir, data_dir_name)
        DOC_DIR = os.path.join(args.doc_dir, data_dir_name)
        INPUT_FILE = os.path.join(args.output_dir, data_dir_name, 'trajectories_cleaned.csv')
